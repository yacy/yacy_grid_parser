...
bindhost="127.0.0.1"
callhost="localhost"
appname="YaCy Grid Parser"
containername=yacy-grid-parser
...
if [ ${containerRuns} -gt 0 ]; then
  ...
  docker run -d --restart=unless-stopped -p ${bindhost}:8500:8500 \
...
echo "To get the app status, open http://${callhost}:8500/yacy/grid/mcp/info/status.json"
