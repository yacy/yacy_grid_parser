...
public class Peers extends TreeMap<byte[], Peer> implements Serializable {
    ...
    public void bootstrap(final String[] peers, int selection) {
                ...
                Thread t0 = new Thread("Peers.bootstrap-" + bp) {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            Data.logger.info("bootstrap with peer " + bp + ": added " + (c1 - c0) + " peers");
                        } catch (final IOException e) {
                            Data.logger.info("bootstrap with peer " + bp + ": FAILED - " + e.getMessage());
                        }
                    }
                };
                t0.start();
        ...
        lastBootstrap = System.currentTimeMillis();
        Data.logger.info("bootstrap finished: " + this.size() + " peers");
    }
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        for (Peer p: nodes) {
            Data.logger.info(p.get(Peer.Schema.fullname) + " - " + p.get(Peer.Schema.address));
        }
